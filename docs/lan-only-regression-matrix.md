...
Latest repository verification: **2026-07-17**, macOS arm64, after the Linux `subnetdesk.service` package and legacy-unit migration changes. The final unified `cargo check --workspace --all-targets --features flutter` and `cargo test --workspace --features flutter` passed. The suite passed **181 tests**, failed none, and explicitly ignored one Enigo key-state test because it requires an interactive desktop plus input-injection permission. Shell syntax checks, the dedicated Linux service contract gate, LAN-only residual gate, SubnetDesk branding/icon manifest gate, and nested-repository diff checks passed.
...
| Linux service | DEB, RPM/SUSE, and Arch packages install and control `subnetdesk.service`; upgrades stop/disable/remove the legacy `rustdesk.service` unit | `scripts/check_linux_service_branding.sh`, invoked by the SubnetDesk branding gate | Source/package contract passed; installed-package lab pending |
